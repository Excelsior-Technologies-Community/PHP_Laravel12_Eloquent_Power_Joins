...
<form action="{{ route('orders.update', $order->id) }}" method="POST" id="orderForm">
                ...
                <div class="col-md-6">
                    <select name="products[{{ $index }}][id]" class="form-select product-select" required>
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" data-price="{{ $product->price }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }} - ₹{{ $product->price }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <input type="number" name="products[{{ $index }}][quantity]" class="form-control qty-input" placeholder="Quantity" min="1" value="{{ $item->quantity }}" required>
                </div>
                <div class="col-md-2">
    ...
    <div class="text-end">
        <button type="submit" class="btn btn-glow btn-lg"><i class="bi bi-check-lg"></i> Update Order</button>
    </div>
</form>
